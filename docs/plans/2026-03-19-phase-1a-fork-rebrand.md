...
**Goal:** Hard-fork OpenLobster master into the MyPal repo and rebrand all references from OpenLobster/OpenClaw to MyPal.

**Architecture:** Clone OpenLobster's Go monorepo (backend + SolidJS frontend, single-binary embed) into the MyPal repo, then systematically rename the binary, CLI commands, environment variable prefix, module path, frontend branding, and Docker artifacts. Verify the build and tests still pass after each rename step.

**Tech Stack:** Go 1.25+, SolidJS + Vite, pnpm workspaces, Turbo, gqlgen, GORM, Docker multi-stage build

---

## Context: OpenLobster Repo Structure

```
apps/
  backend/
    cmd/openlobster/         # CLI entry point (Cobra)
      main.go                # Binary name "openlobster"
    ...
    go.mod                   # module github.com/Neirth/OpenLobster/apps/backend
    go.sum
  frontend/
    src/                     # SolidJS app (components, graphql, hooks, views, stores, styles)
    package.json             # @openlobster/frontend
    vite.config.ts
    index.html
  ui/                        # Shared component library
    package.json             # @openlobster/ui
...
- Go module path: `github.com/Neirth/OpenLobster/apps/backend`
- Binary name: `openlobster`
- Env var prefix: `OPENLOBSTER_*`
- pnpm package names: `@openlobster/*`
...
### Task 1: Import OpenLobster Source

**Files:**
- Create: All OpenLobster source files in MyPal repo root

**Step 1: Clone OpenLobster into a temporary directory**

```bash
git clone --depth 1 https://github.com/Neirth/OpenLobster.git /tmp/openlobster-fork
```

**Step 2: Copy source into MyPal repo (excluding .git)**

```bash
rsync -av --exclude='.git' /tmp/openlobster-fork/ /Users/heidornj/Code/MyPal/
```

This preserves our existing `docs/` directory and git history while bringing in all OpenLobster source.

**Step 3: Verify the file tree landed correctly**

```bash
ls apps/backend/cmd/openlobster/main.go  # should exist
...
```bash
git add -A
git commit -m "Fork OpenLobster master as MyPal base"
```
...
```
module github.com/Neirth/OpenLobster/apps/backend
```
to:
```
...
```bash
cd apps/backend
find . -name '*.go' -exec sed -i '' 's|github.com/Neirth/OpenLobster/apps/backend|github.com/BangRocket/MyPal/apps/backend|g' {} +
```

**Step 3: Verify no stale imports remain**

```bash
grep -r "Neirth/OpenLobster" apps/backend/ --include='*.go' | head -20
```
...
**Files:**
- Rename: `apps/backend/cmd/openlobster/` → `apps/backend/cmd/mypal/`
- Modify: `apps/backend/cmd/mypal/main.go` (binary name, version strings, help text)
- Modify: `apps/backend/cmd/mypal/serve/*.go` (any "openlobster" references in help/logging)
...
```bash
mv apps/backend/cmd/openlobster apps/backend/cmd/mypal
```

**Step 2: Update main.go**

Replace all "openlobster" / "OpenLobster" references with "mypal" / "MyPal" in:
- `apps/backend/cmd/mypal/main.go`
- All files in `apps/backend/cmd/mypal/serve/`

Search for: `openlobster`, `OpenLobster`, `open-lobster`, `open_lobster`

**Step 3: Update Dockerfile build path**

In `.docker/Dockerfile.static`, change:
```
go build -o /app/openlobster ./cmd/openlobster
```
to:
```
...
```bash
git add -A
git commit -m "Rename binary and CLI from openlobster to mypal"
```
...
- Modify: All Go files that reference `OPENLOBSTER_*` env vars
...
**Step 1: Find all OPENLOBSTER_ references**

```bash
grep -r "OPENLOBSTER" --include='*.go' --include='*.yaml' --include='*.yml' --include='*.env*' --include='*.md' --include='*.json' -l
```

**Step 2: Replace OPENLOBSTER_ with MYPAL_ in all files**

The config system in `config.go` binds env vars with a prefix. Find the prefix definition and change it:
```go
// Before
envPrefix = "OPENLOBSTER"
// After
envPrefix = "MYPAL"
```
...
```bash
grep -rl "OPENLOBSTER" --include='*.go' --include='*.yaml' --include='*.yml' --include='*.env*' --include='*.md' --include='*.json' --include='*.ts' --include='*.tsx' | xargs sed -i '' 's/OPENLOBSTER/MYPAL/g'
```

**Step 3: Verify no stale references**

```bash
grep -r "OPENLOBSTER" --include='*.go' --include='*.yaml' --include='*.yml' --include='*.env*' | head -20
```
...
```bash
git add -A
git commit -m "Rename env var prefix from OPENLOBSTER_ to MYPAL_"
```
...
**Files:**
- Modify: `apps/frontend/package.json` (`@openlobster/frontend` → `@mypal/frontend`)
- Modify: `apps/ui/package.json` (`@openlobster/ui` → `@mypal/ui`)
- Modify: Root `package.json` (any `@openlobster/*` references)
- Modify: `apps/frontend/index.html` (page title)
- Modify: `apps/frontend/src/` files (any "OpenLobster" branding strings)
- Modify: All `*.ts`, `*.tsx`, `*.json` files importing `@openlobster/*`

**Step 1: Update package.json names**

In each `package.json`, replace `@openlobster/` with `@mypal/`.

**Step 2: Update all TypeScript/TSX imports**

```bash
grep -rl "@openlobster/" --include='*.ts' --include='*.tsx' --include='*.json' | xargs sed -i '' 's/@openlobster\//@mypal\//g'
```

**Step 3: Update user-visible branding strings**

Search for "OpenLobster", "Open Lobster", "openlobster" in:
...
```bash
git add -A
git commit -m "Rebrand frontend from OpenLobster to MyPal"
```
...
- Change binary references from `openlobster` to `mypal`
- Update LABEL metadata if present
- Verify ENTRYPOINT/CMD uses the new binary name

**Step 2: Update docker-compose files**

- Service names: `openlobster` → `mypal`
- Image names: `openlobster:latest` → `mypal:latest`

**Step 3: Update CI/CD if present**

Search `.github/` for any workflow files referencing "openlobster" and update.
...
**Files:**
- Modify: Any remaining files with "OpenLobster", "OpenClaw", "Neirth" references
...
```bash
grep -ri "openlobster\|openclaw\|neirth" --include='*.go' --include='*.ts' --include='*.tsx' --include='*.json' --include='*.yaml' --include='*.yml' --include='*.toml' -l
```
...
- Note: "Forked from OpenLobster (Neirth/OpenLobster)"
...
```bash
git add -A
git commit -m "Clean up remaining OpenLobster/OpenClaw references"
```
...
```bash
grep -ri "openlobster" --include='*.go' --include='*.ts' --include='*.tsx' --include='*.json' --include='*.yaml' --include='*.yml' | grep -v "node_modules" | grep -v ".git"
```
...
```bash
git tag v0.0.1-fork -m "MyPal: hard fork of OpenLobster master"
```
...
|------|-------------|----------|
| 1 | Import OpenLobster source | Merge conflicts with existing docs |
